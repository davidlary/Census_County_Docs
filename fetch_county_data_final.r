...
fetch_county_data <- function(crosswalk, parallel = TRUE, num_cores = NULL) {
  ...
  decennial_vars <- crosswalk %>%
    select(std_name, matches("dec_\\d{4}_var")) %>%
    pivot_longer(-std_name, names_to = "source", values_to = "variable") %>%
    ...
    decennial_results <- progressr::with_progress({
      ...
      future.apply::future_lapply(decennial_years, function(year) {
        ...
        if (length(vars) == 0) {
          ...
          return(tibble(year = integer(), GEOID = character(), NAME = character(), variable = character(), value = numeric(), source = character(), std_name = character()))
        }
          ...
          if (!is.null(result)) {
            result %>%
              mutate(
                ...
                std_name = var_std_map[variable] # Add standardized name directly
              ) %>%
              select(year, GEOID, NAME, variable, value, source, std_name)
          } else {
            tibble(year = integer(), GEOID = character(), NAME = character(), variable = character(), value = numeric(), source = character(), std_name = character())
          }
        })
            ...
            future.apply::future_lapply(1:nrow(var_pairs), function(i) {
              ...
              if (!is.null(result)) {
                ...
                result %>%
                  ...
                  select(year, GEOID, NAME, variable, value = estimate, source, std_name)
              } else {
      ...
      for (i in 1:nrow(problematic_2022_data)) {
        ...
        if (nrow(replacement_data) > 0) {
          ...
          acs_data <- acs_data %>%
            mutate(
              value = ifelse(
                year == 2022 & GEOID == problem_row$GEOID & std_name == problem_row$std_name,
