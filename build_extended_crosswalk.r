...
build_extended_crosswalk <- function(include_nhgis = TRUE, include_places = TRUE) {
  ...
  base_social_determinants <- tribble(
    ~std_name, ~category, ~description, ~acs_var, ~nhgis_var, ~cdc_places_var,
  ...
  nhgis_determinants <- if (include_nhgis) {
    ...
    tibble(std_name = character(), category = character(), description = character(), 
  ...
  distinct(std_name, .keep_all = TRUE)
  ...
  if (include_places) {
    additional_places_vars <- cdc_places_vars %>%
      anti_join(social_determinants, by = c("short_name" = "std_name")) %>%
      mutate(
        std_name = short_name,
        category = case_when(
      ...
      select(std_name, category, description, acs_var, nhgis_var, cdc_places_var)
  } else {
    ...
    base_crosswalk %>% 
      mutate(
        ...
        description = case_when(
          std_name == "total_population" ~ "Total population",
          std_name == "median_age" ~ "Median age (years)",
          std_name == "male_population" ~ "Male population",
          std_name == "female_population" ~ "Female population",
          TRUE ~ NA_character_
        ),
        nhgis_var = case_when(
          std_name == "total_population" ~ "TOTPOP",
          std_name == "median_age" ~ "MEDAGE",
          std_name == "male_population" ~ "MALE",
          std_name == "female_population" ~ "FEMALE",
          TRUE ~ NA_character_
        ),
  ...
  data_dictionary <- extended_crosswalk %>%
    mutate(
      variable_type = case_when(
        str_detect(std_name, "pct$|rate$") ~ "Percentage (0-100)",
        str_detect(std_name, "median|mean") ~ "Currency ($) or Count",
        str_detect(std_name, "population") ~ "Count",
        TRUE ~ "Varies"
      ),
    ...
    select(
      std_name, description, category, variable_type, 
